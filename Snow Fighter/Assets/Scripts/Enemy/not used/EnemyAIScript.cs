...
public class EnemyAIScript : MonoBehaviour
{
   ...
   //     Debug.Break();
        ...
        snow.Initialize(power, snowStartTrans.position, snowStartTrans.rotation, transform, playerTrans.position) ;
        animator.SetBool("IsReadyToThrow", false);
        animator.SetBool("isThrowing", false);
    }

    public void throwEnemySnow()
    {
        float projectileTimeToTarget = distanceToTarget / power;

        Vector3 projectedTarget = targetPos + targetDirection * projectileTimeToTarget;
        projectedTarget.y += 1; //aim at center of target if 2*m high

        snow = GetComponentInChildren<SnowBallScript>();
        if (snow == null) return;
        snow.gameObject.transform.SetParent(null);
        //snow.Initialize(power, snowStartTrans.position, snowStartTrans.rotation, transform, playerTrans.position);
        snow.Initialize(transform);
        snow.transform.LookAt(projectedTarget);
        Rigidbody rb = snow.GetComponent<Rigidbody>();
        rb.isKinematic = false;
        rb.velocity = snow.transform.forward * power;
        rb.useGravity = true;
